...
# from rich.live import Live  # Unused import
from rich.layout import Layout

# from rich.columns import Columns  # Unused import
...
@click.option(
    ...
    "--type",
    "error_type",
    shell_complete=error_type_completion,
...
@click.option(
    ...
    "--copy",
    "copy_format",
    shell_complete=clipboard_format_completion,
    help="Copy to clipboard with format (plain/markdown/code/json)",
)
@click.option("-o", "--output", "output_path", type=click.Path(), help="Save to file")
@click.option(
    "--format",
    "output_format",
    shell_complete=output_format_completion,
    default="text",
    help="Output format (text/json/markdown/html/yaml)",
)
...
@click.option(
    "--theme", default="default", help="Output theme (default/dark/light/custom)"
)
...
def show(limit: int, filter_type: Optional[str]):
    ...
    for entry in entries:
        ...
        preview = (
            entry["error_content"][:50] + "..."
            if len(entry["error_content"]) > 50
            else entry["error_content"]
        )
...
def list():
    ...
    for p in plugins:
        ...
        table.add_row(
            p["name"], f"[{status_color}]{status}[/{status_color}]", p["description"]
        )
...
def _process_error_enhanced(
    ...
    with Progress(
        ...
        plugin_results = cli_handler.plugins.run_all(
            "analyze",
            {
...
def _display_results_enhanced(result: Dict[str, Any], verbose: bool):
    ...
    if len(result["prompt"]) > 2000:
        ...
        body_content = Syntax(
            result["prompt"], "markdown", theme="monokai", line_numbers=False
        )
    else:
...
def _interactive_mode_enhanced(cli_handler: CCDebugCLI):
    ...
    while True:
        ...
        with console.status("Analyzing error...", spinner="dots"):
            result = _process_error_enhanced(
                cli_handler, error_content, None, None, True
            )
        ...
        choice = Prompt.ask(
            "Choose an option", choices=["c", "s", "r", "n", "q"], default="n"
        )

        if choice == "c":
            format_choice = Prompt.ask(
                "Format", choices=CLIPBOARD_FORMATS, default="markdown"
            )
            clipboard_content = cli_handler.formatter.format_for_clipboard(
            ...
            format_choice = Prompt.ask(
                "Format", choices=OUTPUT_FORMATS, default="markdown"
            )
    ...
    with Progress(
        ...
        for file_path in all_files:
            ...
            try:
                content = Path(file_path).read_text(encoding="utf-8")
                result = _process_error_enhanced(
                    cli_handler, content, error_type, None, False
                )
...
def _show_statistics(cli_handler: CCDebugCLI):
    ...
    if stats.get("types"):
        ...
        for error_type, count in sorted(
            stats["types"].items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (count / total) * 100
            bar = "█" * int(percentage / 2)
            type_chart.add_row(error_type, str(count), f"{bar} {percentage:.1f}%")
