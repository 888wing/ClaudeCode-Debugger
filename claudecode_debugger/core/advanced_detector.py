...
class AdvancedErrorDetector:
    ...
    def detect_multi_label(self, error_text: str, 
                          confidence_threshold: float = 0.3) -> List[ErrorMatch]:
        """
        ...
        # Score each category
        category_scores = defaultdict(lambda: {'score': 0, 'matches': [], 'info': {}})
        
        for pattern in self.patterns:
            matches = list(pattern.findall(error_text)) if pattern.extract_groups else []
                ...
                if pattern.extract_groups and matches:
                    for i, group_name in enumerate(pattern.extract_groups):
                        if isinstance(matches[0], tuple) and i < len(matches[0]):
                            if group_name not in category_scores[pattern.category]['info']:
                                category_scores[pattern.category]['info'][group_name] = []
                            category_scores[pattern.category]['info'][group_name].extend(
                                [m[i] for m in matches if isinstance(m, tuple)]
                            )
        ...
        if total_score > 0:
            for category, data in category_scores.items():
                ...
                if confidence >= confidence_threshold:
                    # Determine severity
                    severity = self._calculate_severity(error_text, category, confidence)
    ...
    def extract_comprehensive_info(self, error_text: str) -> Dict[str, Any]:
        ...
        info = {
            'error_categories': [(m.category.value, m.confidence) for m in error_matches],
    ...
    def _extract_variables(self, text: str) -> Set[str]:
        ...
        for pattern in patterns:
            matches = re.findall(pattern, text, re.IGNORECASE)
            variables.update(match for match in matches if match and match.isidentifier())
    ...
    def _extract_environment_info(self, text: str) -> Dict[str, Any]:
        ...
        env_vars = re.findall(r'(?:process\.env\.|ENV\[|os\.environ\[)[\'"](\w+)[\'"]', text)
        if env_vars:
            env_info['env_variables'] = list(set(env_vars))
        
        return env_info
    
    def _calculate_severity(self, text: str, category: ErrorCategory, 
    ...
    def _enhance_with_ml(self, text: str, results: List[ErrorMatch]) -> List[ErrorMatch]:
    ...
    def stream_detect(self, file_path: Union[str, Path], 
                     chunk_size: int = 8192) -> List[ErrorMatch]:
        """
    ...
    def _merge_matches(self, matches: List[ErrorMatch]) -> List[ErrorMatch]:
        ...
        for category, cat_matches in category_matches.items():
            ...
            severity = max((m.severity for m in cat_matches), 
