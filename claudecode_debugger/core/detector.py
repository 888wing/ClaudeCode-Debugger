...
class ErrorDetector:
    ...
    def __init__(self):
        ...
        self.compiled_patterns = {
            error_type: [re.compile(pattern, re.IGNORECASE | re.MULTILINE) 
    ...
    def get_severity(self, error_text: str, error_type: str) -> str:
        ...
        # Critical indicators
        critical_keywords = [
            'build failed', 'compilation failed', 'fatal error',
            'out of memory', 'segmentation fault', 'kernel panic',
            'data loss', 'security vulnerability', 'authentication failed'
        ]
        
        # High severity indicators  
        high_keywords = [
            'error', 'exception', 'failed', 'cannot', 'undefined',
            'null reference', 'type error', 'syntax error'
        ]
        ...
        if any(keyword in lower_text for keyword in critical_keywords):
            return 'critical'
        elif error_type in ['memory', 'build'] or any(keyword in lower_text for keyword in high_keywords):
            return 'high'
