...
class PromptGenerator:
    ...
    def __init__(self, template_dir: Optional[Path] = None, use_advanced: bool = True):
        ...
        if self.use_advanced:
            ...
            # Add custom filters
            self.env.filters["capitalize_first"] = lambda s: (
                s[0].upper() + s[1:] if s else ""
            )
    ...
    def _prepare_context(
        self, error_text: str, error_type: str, error_info: Optional[Dict] = None
    ) -> Dict[str, Any]:
...
"""
    ...
    def _find_best_template(
        self, error_type: str, error_text: str, error_info: Optional[Dict] = None
    ) -> str:
        ...
        if any(
            ...
            for kw in ["docker", "container", "kubernetes", "k8s", "pod"]
        ):
        ...
        if any(
            kw in error_text.lower() for kw in ["memory", "heap", "oom", "outofmemory"]
        ):
    ...
    def _determine_severity(
        self, error_type: str, error_info: Optional[Dict] = None
    ) -> str:
