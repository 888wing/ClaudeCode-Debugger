...
class ErrorMLClassifier:
    ...
    def _extract_structural_features(self, text: str) -> np.ndarray:
        ...
        features = [
            ...
            np.std([len(line) for line in lines]) if len(lines) > 1 else 0,  # Line length variance
            sum(1 for line in lines if line.strip().startswith('at ')),  # Stack trace lines
    ...
    def train(self, training_examples: List[TrainingExample], 
        ...
        X_train, X_val, y_cat_train, y_cat_val, y_sev_train, y_sev_val = \
            train_test_split(X, y_categories, y_severity, 
    ...
    def _evaluate(self, X_val: np.ndarray, y_cat_val: np.ndarray, 
    ...
    def predict(self, text: str) -> Tuple[List[Tuple[str, float]], str, float]:
        ...
        # Overall confidence
        overall_confidence = np.mean([c[1] for c in categories[:3]]) if categories else 0.0
        
        return categories, severity, overall_confidence
    
    def predict_batch(self, texts: List[str]) -> List[Tuple[List[Tuple[str, float]], str, float]]:
    ...
    def update_model(self, new_examples: List[TrainingExample]):
        ...
        logger.warning("Incremental learning not fully implemented. Retraining with new examples only.")
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        custom_features = [
            'text_length', 'line_count', 'word_count', 'avg_word_length',
            'error_keywords', 'warning_keywords', 'critical_keywords',
            'network_keywords', 'memory_keywords', 'type_keywords',
            'typescript_patterns', 'stack_trace_patterns', 'line_number_patterns',
            'url_patterns', 'error_type_patterns', 'file_reference_patterns',
            'max_line_length', 'line_length_variance', 'stack_trace_lines',
            'colon_lines', 'bracket_count'
        ]
        ...
        # Sort by importance
        return dict(sorted(avg_importance.items(), 
                          key=lambda x: x[1], 
                          reverse=True)[:50])  # Top 50 features