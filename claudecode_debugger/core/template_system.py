        ...
        with self.lock:
            if key in self.cache:
                ...
                if (
                    ...
                    and (datetime.now().timestamp() - timestamp) < self.ttl
                ):
                    return data
        return None
...
class AdvancedTemplateSystem:
    ...
    def _add_custom_filters(self):
        ...
        filters = {
            ...
            "truncate_middle": lambda s, n=50: (
                s if len(s) <= n else s[: n // 2] + "..." + s[-(n // 2) :]
            ),
            ...
            "severity_color": lambda s: {
                "error": "🔴",
                "warning": "🟡",
                "info": "🔵",
            }.get(s.lower(), "⚪"),
            "format_timestamp": lambda ts: datetime.fromtimestamp(ts).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
    ...
    def _load_template(self, template_path: Path):
        ...
        try:
            ...
            self.template_versions[template_name] = hashlib.md5(
                content.encode()
            ).hexdigest()
    ...
    def _resolve_dependencies(self):
        ...
        for template_name in self.templates:
            ...
            if self._has_circular_dependency(template_name, visited):
                print(
                    f"Warning: Circular dependency detected for template {template_name}"
                )
        ...
        try:
            ...
            if "extends" in template_data:
                if template_data["extends"] not in self.templates:
                    errors.append(
                        f"Parent template '{template_data['extends']}' not found"
                    )
