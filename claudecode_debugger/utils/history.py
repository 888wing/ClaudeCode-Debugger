...
class HistoryManager:
    ...
    def add_entry(self, entry: Dict[str, Any]):
        ...
        metadata = {
            ...
            if k not in ['timestamp', 'error_content', 'error_type', 'severity', 'agent', 'output_format']
        }
        
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
                INSERT INTO history 
                (timestamp, error_content, error_type, severity, agent, output_format, metadata)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                entry['timestamp'],
                entry['error_content'],
    ...
    def get_entries(self, limit: int = 10, filter_type: Optional[str] = None,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            last_24h = conn.execute('''
                ...
                WHERE timestamp >= ?
            ''', ((now - timedelta(hours=24)).isoformat(),)).fetchone()[0]
        ...
        return {
            ...
            'success_rate': ((total - critical_count) / total * 100) if total > 0 else 0
        }
